...
import Managers.PortfolioHistoryManager;
...
// Create a ChartPanel displaying the value of the portfolio over time.
    ...
    public PortfolioValueChartPanel(Portfolio portfolio){
    ...
    public XYSeries getData() {
        ...
        PortfolioHistoryManager historyManager = PortfolioHistoryManager.getInstance();

        Queue<Double> historyQueue = historyManager.get(portfolio);

        int i = historyQueue.size();
        for (Double history : historyQueue) {
            series.add(-i, history);
            i--;
        }
